...
高阶组件的作用：共享组件之间的**状态逻辑！！！！！**不是单纯共享数据，而是共享状态，以及状态相关的逻辑！

高阶组件可以看做创建包装类的函数。

高阶组件的实现，就是将两个组件之间需要复用的状态及其逻辑提取出来，将目标数据作为参数传入被包装类中。（与将公共部分用函数提取出来，在函数中返回目标值，由被包装类直接调用函数实现的功能差不多，但高阶组件是一种类似于包装类的设计模式，封装性更好，且公共部分的处理过程可以是有状态的，普通的获取数据的函数没有状态。应该是这样没错，和hook差不多，官网的自定义hook就是有状态的，可以试下高阶组件能否实现这种效果，写个订阅模式就可验证）
...
可以想象，在一个大型应用程序中，这种订阅 `DataSource` 和调用 `setState` 的模式将一次又一次地发生。我们需要一个抽象，允许我们在一个地方定义这个逻辑，并在许多组件之间共享它。这正是高阶组件擅长的地方。
