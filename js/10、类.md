...
普通 构造函数如果不使用 new 调用，那么就会以当前 this（通常是 window）作为内部对象。
...
~~**不能在类块中给原型添加原始值或对象作为成员数据**~~  ES6已经支持了，且作为实例属性而非原型属性
...
可以在类上定义静态方法，和静态变量。这些方法通常用于执行不特定于实例的操作，也不要求存在类的实例。与原型成员类似，**静态方法是共享的**，**静态方法不保存在原型上**
